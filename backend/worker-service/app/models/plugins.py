...
class JobStatusType(TypeDecorator):
    """TypeDecorator to ensure enum values (not names) are stored and read from database"""
    impl = String(20)
    cache_ok = True
...
class Job(Base):
    ...
    status: Mapped[JobStatus] = mapped_column(JobStatusType(), default=JobStatus.PENDING)
