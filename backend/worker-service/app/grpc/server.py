"""gRPC server for worker microservice"""
import asyncio
import grpc
...
async def serve():
    ...
    if not worker_pb2_grpc:
        logger.error("Proto files not generated. Run: python -m grpc_tools.protoc --proto_path=proto --python_out=proto --grpc_python_out=proto proto/worker.proto")
        return
