...
class MicroserviceService:
    ...
    def apply_templating(self, directory: Path, context: Dict) -> None:
        """
        Recursively render all files in a directory using Jinja2.
        ...
        for root, _, files in os.walk(directory):
            for file in files:
                ...
                if ".git" in root or file.endswith(('.png', '.jpg', '.jpeg', '.gif', '.ico', '.pdf', '.zip')):
                    continue
                ...
                try:
                    ...
                    logger.debug(f"Rendered template: {relative_path}")
                except TemplateError as e:
                    logger.warning(f"Failed to render template {relative_path}: {e}")
                except Exception as e:
                    logger.error(f"Error processing file {relative_path} for templating: {e}")
    ...
    def create_repository_from_template(
        ...
        if "deployment_name" not in render_context:
            render_context["deployment_name"] = repo_name
        ...
        try:
            ...
            # Step 2.5: Apply templating
            logger.info(f"Applying templating to {extracted_dir}")
            self.apply_templating(extracted_dir, render_context)
